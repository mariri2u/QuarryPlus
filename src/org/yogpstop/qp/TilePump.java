...
public class TilePump extends APacketTile implements ITankContainer, IPowerReceptor {
	...
	public String[] C_getNames() {
		...
		for (int i = 0; i < ret.length; i++) {
			index = this.liquids.indexOf(LiquidDictionary.getCanonicalLiquid(this.mapping[i]));
	...
	String incl(int side) {
		...
		int index = this.liquids.indexOf(LiquidDictionary.getCanonicalLiquid(this.mapping[side])) + 1;
		if (index == this.liquids.size()) index = 0;
		return this.mapping[side] = LiquidDictionary.findLiquidName(this.liquids.get(index));
	}
