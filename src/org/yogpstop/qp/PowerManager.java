...
public class PowerManager {
	...
	static void configure0(IPowerProvider pp) {
		pp.configure(0, 0, 0, 0, Integer.MAX_VALUE);
	}

	private static void configure(IPowerProvider pp, double CE, byte E, byte U, double CU, double NR, double XR, double BP, double MS, byte pump) {
		pp.configure(0, (int) (NR / (U * CU + 1)), (int) (XR * Math.pow(CE, E) / (U * CU + 1)), (int) (BP * Math.pow(CE, E) / (U * CU + 1)),
				(int) (MS * Math.pow(CE, E) / (U * CU + 1) + (pump > 0 ? ((65536 * PL_BP / ((pump * PL_CU + 1))) + (1020 * PF_BP / ((pump * PF_CU + 1)))) : 0)));
	}

	private static void configure15(IPowerProvider pp, double CE, byte E, byte U, double CU, double NR, double XR, double BP, double MS, byte pump) {
	...
	static void configureB(IPowerProvider pp, byte E, byte U, byte pump) {
		configure15(pp, B_CE, E, U, B_CU, B_NR, B_XR, B_BP, B_MS, pump);
	}

	static void configureW(IPowerProvider pp, byte E, byte U, byte pump) {
		configure15(pp, W_CE, E, U, W_CU, W_NR, W_XR, W_BP, W_MS, pump);
	}

	static void configureL(IPowerProvider pp, byte E, byte U) {
		configure(pp, L_CE, E, U, L_CU, L_NR, L_XR, L_BP, L_MS, (byte) 0);
	}

	static void configureF(IPowerProvider pp, byte E, byte U, byte pump) {
		configure(pp, F_CE, E, U, F_CU, F_NR, F_XR, F_BP, F_MS, pump);
	}

	static void configureR(IPowerProvider pp, byte E, byte U) {
		configure(pp, R_CE, E, U, R_CU, R_NR, R_XR, 25, R_MS, (byte) 0);
	}

	private static boolean useEnergy(IPowerProvider pp, double BP, float H, double CSP, byte U, double CU) {
	...
	private static boolean useEnergy(IPowerProvider pp, double BP, byte U, double CU) {
	...
	private static boolean useEnergy(IPowerProvider pp, double BP1, long amount1, double BP2, long amount2, byte U, double CU1, double CU2) {
	...
	private static double useEnergy(IPowerProvider pp, double D, double BP, byte U, double CU) {
		float pw = (float) Math.min(2 + pp.getEnergyStored() / 500, (D - 0.1) * BP / (U * CU + 1));
		pw = pp.useEnergy(pw, pw, true);
		return pw * (U * CU + 1) / BP + 0.1;
	}

	private static float useEnergy(IPowerProvider pp, double BP, byte U, double CU, byte F, double CF, boolean S, double CS, byte E, double CE) {
	...
	static boolean useEnergyB(IPowerProvider pp, float H, double CSP, byte U, BreakType t) {
		return useEnergy(pp, t == BreakType.Quarry ? B_BP : W_BP, H, CSP, U, t == BreakType.Quarry ? B_CU : W_CU);
	}

	static boolean useEnergyF(IPowerProvider pp, byte U) {
		return useEnergy(pp, F_BP, U, F_CU);
	}

	static boolean useEnergyR(IPowerProvider pp, int energy, byte U) {
		return useEnergy(pp, energy, U, R_CU);
	}

	static boolean useEnergyP(IPowerProvider pp, byte U, long liquids, long frames) {
		return useEnergy(pp, PL_BP, liquids, PF_BP, frames, U, PL_CU, PF_CU);
	}

	static double useEnergyH(IPowerProvider pp, double distance, byte U) {
		return useEnergy(pp, distance, H_BP, U, H_CU);
	}

	static float useEnergyL(IPowerProvider pp, byte U, byte F, boolean S, byte E) {
