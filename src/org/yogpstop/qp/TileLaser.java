...
public class TileLaser extends TileEntity implements IPowerReceptor, IActionReceptor, IMachine, IEnchantableTile {
	...
	protected IPowerProvider powerHandler = PowerFramework.currentFramework.createPowerProvider();
	...
	protected boolean isValidTable() {
		...
		for (ILaserTarget lt : this.laserTargets)
			if (lt == null || lt.isInvalid() || !lt.hasCurrentWork()) return false;
		return true;
	}
			...
			for (int y = minY; y <= maxY; ++y) {
				for (int z = minZ; z <= maxZ; ++z) {
					...
					if (tile instanceof ILaserTarget) {
						...
						if (table.hasCurrentWork() && !table.isInvalid()) {
