		...
		if (tile != null) {
			...
			if (liquid1 != null && liquid1.amount > 0) {
				...
				if (list1 != null) {
					...
					GL11.glCallList(list1[(int) (liquid1.amount / (float) tile.buf * (LiquidRenderer.DISPLAY_STAGES - 1))]);
				}
			}
			...
			if (liquid2 != null && liquid2.amount > 0) {
				...
				if (list2 != null) {
					...
					GL11.glCallList(list2[(int) (liquid2.amount / (float) tile.buf * (LiquidRenderer.DISPLAY_STAGES - 1))]);
					GL11.glPopMatrix();
				}
			}
			...
			if (liquidResult != null && liquidResult.amount > 0) {
				...
				if (list3 != null) {
					...
					GL11.glCallList(list3[(int) (liquidResult.amount / (float) tile.buf * (LiquidRenderer.DISPLAY_STAGES - 1))]);
					GL11.glPopMatrix();
				}
			}
			GL11.glPopAttrib();
