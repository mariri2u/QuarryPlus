...
public class GuiP_List extends GuiScreenA {
	...
	@Override
	public void initGui() {
		this.controlList.add(new GuiButton(-4, this.width / 2 - 160, this.height - 26, 100, 20, StatCollector.translateToLocal("pp.change")));
		this.controlList.add(new GuiButton(-1, this.width / 2 - 50, this.height - 26, 100, 20, StatCollector.translateToLocal("gui.done")));
		this.controlList.add(new GuiButton(-5, this.width / 2 + 60, this.height - 26, 100, 20, StatCollector.translateToLocal("pp.copy")));
		this.controlList.add(new GuiButton(-2, this.width * 2 / 3 + 10, 45, 100, 20, StatCollector.translateToLocal("tof.addnewore") + "("
				+ StatCollector.translateToLocal("tof.manualinput") + ")"));
		this.controlList.add(new GuiButton(-3, this.width * 2 / 3 + 10, 20, 100, 20, StatCollector.translateToLocal("tof.addnewore") + "("
				+ StatCollector.translateToLocal("tof.fromlist") + ")"));
		this.controlList.add(this.delete = new GuiButton(PacketHandler.CtS_REMOVE_MAPPING, this.width * 2 / 3 + 10, 70, 100, 20, StatCollector
				.translateToLocal("selectServer.delete")));
		this.controlList.add(this.top = new GuiButton(PacketHandler.CtS_TOP_MAPPING, this.width * 2 / 3 + 10, 95, 100, 20, StatCollector
				.translateToLocal("tof.top")));
		this.controlList.add(this.up = new GuiButton(PacketHandler.CtS_UP_MAPPING, this.width * 2 / 3 + 10, 120, 100, 20, StatCollector
				.translateToLocal("tof.up")));
		this.controlList.add(this.down = new GuiButton(PacketHandler.CtS_DOWN_MAPPING, this.width * 2 / 3 + 10, 145, 100, 20, StatCollector
				.translateToLocal("tof.down")));
		this.controlList.add(this.bottom = new GuiButton(PacketHandler.CtS_BOTTOM_MAPPING, this.width * 2 / 3 + 10, 170, 100, 20, StatCollector
